        ...
        if ( 0 === l.remaining ) {
          ...
          if ( options.async ) {
            ...
            if ( 'map' === chart.type ) {
              ...
              if ( l.startDuration && options.playAnimation ) {
                if ( 'stock' === chart.type ) {
  ...
  try {
	  ...
	  if(options.data) {
		  for(var key in options.data) {
			  paramArray.push(key + '=' + options.data[key]);
		  }
	  	}
	  if(options.timestamp) {
		  paramString = AmCharts.timestampUrl(url) + (paramArray.length > 0?'&' + paramArray.join('&'):'');
	  } else {
		  paramString = url + (paramArray.length > 0?'?' + paramArray.join('&'):'');
	  }
    request.open(options.type, paramString, options.async );
